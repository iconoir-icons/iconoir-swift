...
echo "// This file was generated by the update.sh script located in the root of this package."
echo "/// This enum provides a list of all the icons provided in the [iconoir website](https://www.iconoir.com)."
echo "/// There are some conditions which the image was named after a keyword in swift, and as such has been replaced"
echo "/// by a placeholder, either prefixed or suffixed with `icon`. For example `import` would become `importIcon`."
echo "/// Any image that begins with a digit would be prefixed with `icon` for example `icon2x2cell`"
